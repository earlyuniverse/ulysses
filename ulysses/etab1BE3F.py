...
class EtaB_1BE3F(ulysses.ULSBase):
    """
    Three-flavoured Boltzmann equation (BE) with one decaying sterile. See arxiv:1112.4528.
    ...
    def EtaB(self):
        ...
        k       = np.real(self.k1)
        y0      = np.array([self.initial_abundance * self.N1Eq(self.zmin) + 0j, 0+0j,0+0j,0+0j], dtype=np.complex128)
