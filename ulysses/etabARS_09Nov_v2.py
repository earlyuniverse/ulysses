...
class EtaB_ARS(ulysses.ULSBase):
    ...
    def EtaB(self):
        ...
        if self._zcut == 1.0:
            ...
            print(colored("Info: User did not provide zcut so no stitching is performed. \n", 'red'))
            rtol = 1e-7
            
            if zosc < 0.1:
                print(colored("Warning: Dimensionless oscillation time smaller than 0.1. Code might become slow. \n", 'red'))
                rtol = 1e-5
            ys = solve_ivp(lambda t, z: self.RHS(t, z, Fmat, self.M2, dMval, Tew, gss, M0, M_mat, Dm2_mat, chi_mat, Lvec, Rvec, acr),
                           [1.e-6, 1], y0, method='BDF', rtol=rtol, atol=1.e-10)
