...
def compute_linearised_coefficients(Fmat, M_mat, chi_mat, Dm2_mat):
...
def lin_RHS(x, y, M1, Tew, M0, Lambda,
    ...
    if kn2 == 0:
        ...
        fact2   = (9*1.20205/(2*np.pi**2)) * (zz**2 * kn2/2)
        N_eq_x  = (3.0/16.0) * zz**2 * kn2
    # Source coefficient in the deviation variable: N_eq * fact = -dN_eq/dx
    fact2_N = N_eq_x * fact
    # fact2/N_eq_x = 12*zeta3/pi^2 is z-independent (both proportional to z^2*K2)
    FACT2_OVER_NEQ = 12.0 * 1.20205 / (np.pi**2)

    # Variable: y = rho_N - N_eq*I  (deviation from equilibrium, not rho/N_eq - I).
    # A_L term drops: it was -d(ln N_eq)/dx * I on the density-matrix block, which
    # exactly cancels the integrating-factor shift when multiplying out P*A_old*P_inv.
    # Cross-sector rescaling: [mu,rho] block fact2 -> fact2/N_eq (= const);
    #                         [rho,mu] block G1,S1 -> G1*N_eq, S1*N_eq.
    A = (M0/Tew) *(h0 * A_ham_0 * x*x/(Tew*Tew) + hLNC * A_ham_LNC + hLNV * A_ham_LNV +
                   A_G0 * G0 + A_S0 * S0 +
                   A_G0_fact2 * G0*FACT2_OVER_NEQ + A_S0_fact2 * S0*FACT2_OVER_NEQ +
                   A_G1*G1*N_eq_x + A_S1*S1*N_eq_x +
                   A_G1_fact2*G1*fact2 + A_S1_fact2*S1*fact2)
    ...
    # Source scaled by N_eq (deviation-variable convention)
    eqtns = (A_regulated@y + R@C_col*fact2_N)# + 0.5*NL@y)
...
def Jac(x, M1, Tew, M0, Lambda,
    ...
    A = (M0/Tew) *(h0 * A_ham_0 * x*x/(Tew*Tew) + hLNC * A_ham_LNC + hLNV * A_ham_LNV +
                               A_G0 * G0 + A_S0 * S0 +
                               A_G0_fact2 * G0*FACT2_OVER_NEQ + A_S0_fact2 * S0*FACT2_OVER_NEQ + A_G1*G1*N_eq_x + A_S1*S1*N_eq_x + A_G1_fact2*G1*fact2 + A_S1_fact2*S1*fact2) 
    #The factors of 2 in front of A_G0_fact2 and A_S0_fact2 and the factors of 0.5 in front of A_G1 and A_S1 are inserted by hand
    #to match with the equations given in arxiv:2407.10560
    ...
    return  np.real(A_regulated), np.real(R@C_col*fact2_N)
...
class EtaB_ARS_3RHN(ulysses.ULSBase):
    ...
    def EtaB(self):
        ...
        x0          = 1e-6
        z0          = self.M1 * x0 / Tew
        Neq0        = 3.0/8.0 if z0 == 0 else (3.0/16.0) * z0**2 * kn(2, z0)
        y0          = np.array([(self.initial_abundance-1.) * Neq0] + [0.] * 8 + [(self.initial_abundance-1.) * Neq0] + [0.]*8 + [0, 0, 0], dtype=np.complex128)
        ...
        Neq         = np.array([(3/16) * (self.M1 * x / Tew)**2 * kn(2, self.M1 * x / Tew) for x in t])
        ...
        if self.plot == True:
            ...
            ax.plot(t, np.abs(rN1_minus_id + Neq), label=r"$N_{N_1}$", linewidth=lw, color = colors[4], ls = '-')
            ax.plot(t, np.abs(rN2_minus_id + Neq), label=r"$N_{N_2}$", linewidth=lw, color = colors[5], ls = '-.')
            ax.plot(t, np.abs(rN3_minus_id + Neq), label=r"$N_{N_3}$", linewidth=lw, color = colors[6], ls = ':')
            ax.plot(t, np.real(rN1_minus_id - rNbar1_minus_id), label=r"$|N_{N_1}-N_{\overline{N}_1}|$", linewidth=lw, color = 'k', ls = '-')
            ax.plot(t, np.real(rN2_minus_id - rNbar2_minus_id), label=r"$|N_{N_2}-N_{\overline{N}_2}|$", linewidth=lw, color = 'k', ls = '-.')
            ax.plot(t, np.real(rN3_minus_id - rNbar3_minus_id), label=r"$|N_{N_3}-N_{\overline{N}_3}|$", linewidth=lw, color = 'k', ls = ':')
            ...
            ax.plot(t, -np.real(rN1_minus_id - rNbar1_minus_id), linewidth=lw, color = 'k', linestyle='-')
            ax.plot(t, -np.real(rN2_minus_id - rNbar2_minus_id), linewidth=lw, color = 'k', linestyle='-.')
            ax.plot(t, -np.real(rN3_minus_id - rNbar3_minus_id), linewidth=lw, color = 'k', linestyle=':')
        ...
        evol_joint = np.array([t,np.abs(rN1_minus_id + Neq),np.abs(rN2_minus_id + Neq), np.abs(rN3_minus_id + Neq), np.real(rN1_minus_id - rNbar1_minus_id), 
                       np.real(rN2_minus_id - rNbar2_minus_id), np.real(rN3_minus_id - rNbar3_minus_id), np.real(muD1), np.real(muD2), np.real(muD3)])
