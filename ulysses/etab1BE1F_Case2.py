...
# Case D2 of arXiv:0907.0205 — kinetic equilibrium of the RHN has been dropped.
import ulysses
import numpy as np
import math
...
def solve(yn, K, zs, fN0=None, method="RK45", max_step=1/300.):
    ...
    fN0: initial distribution function values at zs[0] (one per yn entry).
         None → vanishing IC (zeros). For thermal IC pass
         np.exp(-np.sqrt(zs[0]**2 + yn**2)).
    """
    y0 = fN0 if fN0 is not None else np.zeros_like(yn)
    sol = solve_ivp(D2Nrhs, [zs[0], zs[-1]],
                    y0, args=(yn, K),
...
def Nl_lstore(fn_store, yn_vals, zs):
    """Build and return the RHS of the lepton asymmetry ODE for Case 2."""
    ...
    def D2Lintegrand(yn, data_):
        """Integrand for the lepton asymmetry evolution in Case 2."""
        data = nb.carray(data_, (4,))
        yl, z, Nl, eps = data
        en   = math.sqrt(z * z + yn * yn)
        fun  = eval(z, np.abs(yn))
        fNeq = math.exp(-en)
        p1   = (yn / en) * (4/3) * Nl * fNeq
        p2   = (yn / en) * (-2. * eps * (fun - fNeq))
        return p1 + p2
    D2Lintegrand_ptr = D2Lintegrand.address
    ...
    def ynintegral(yl, data_):
        ...
        integral = dqags(D2Lintegrand_ptr, nlowerlim, 300., data1,
...
class EtaB_1BE1F_Case2(ulysses.ULSBase):
    """
    Boltzmann equations with one decaying sterile, Case D2 of arXiv:0907.0205.
    Kinetic equilibrium of the RHN is dropped; Maxwell-Boltzmann statistics assumed.
    See eqs. 3.25 and 3.27. Flavour oscillations not included.
    """
    ...
    def EtaB(self):
        ...
        fN0      = self.initial_abundance * np.exp(-np.sqrt(zs[0]**2 + yn_vals**2))
