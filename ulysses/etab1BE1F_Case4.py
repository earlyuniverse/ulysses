...
def D4Lintegrand(yn, yl, z, Nl, eps, Nn,):
...
def NLrhs(z, Nl, K, eps, case, highlim = 300, epsrel = 1e-10, epsabs = 1e-10):
    ...
    if case == 4:
        """Case 4"""
        Nn = None
        integral1 = quad(ynintegral, llowerlim, highlim, args=(z, Nl, eps, Nn,  4,), epsabs=epsabs, epsrel=epsrel)
        int = integral1[0]
...
class EtaB_1BE1F_Case4(ulysses.ULSBase):
    ...
    def EtaB(self):
        ...
        solLD4 = solve_ivp(NLrhs, z_span, [0], t_eval=z_eval,
                       args=(K, eps, 4,), method="RK45", atol=1e-10,
                       rtol=1e-10, dense_output=True)
        normfact = 0.013
        return   solLD4.y[-1][-1]* normfact
